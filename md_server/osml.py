...
@dataclass
class Component:
    """HTML Component class for building HTML documents in Python
    ...
    attrs are not validated to be valid and inserted directly in the html tag
    attrs can be valueless by setting them to None

    DangerouslySetInnerHTML can be used to bypass html escape in children
    """
    ...
    def render_component(self, indent: int = 0) -> List[str]:
        print("render component", self.__class__.__name__, self.children)
        ...
        if self.style:
            attrs["style"] = "; ".join([f"{k.replace(';', '')}: {v}" for k, v in self.style.items()])
        # Generate opening tag string
        attrs_string = " ".join(
            # If value is None do not output equal sign or value
            f'{escape(key.replace("_", "-"))}="{escape(value)}"' if value is not None else f'{escape(key.replace("_", "-"))}'
        ...
        if len(self.children) == 1 and isinstance(self.children[0], str) and not isinstance(self.children[0], DangerouslySetInnerHTML):
            return [first_line + escape(self.children[0]) + f"</{name}>"]
        out = [first_line]
        out += render_components(self.children, indent + 2)
        out += [" " * indent + f"</{name}>"]
        return out
