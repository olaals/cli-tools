...
///
/// Additionally:
/// - By default, all task stdout lines are forwarded to the program's stdout
///   (prefixed with `[task]` for clarity).
pub fn setup_long_lived_handlers(
    task: &ScheduledTask,
    stdout: Option<ChildStdout>,
    runtime_tx: mpsc::Sender<RuntimeEvent>,
) {
    let has_stdout_logic = task.progress_on_stdout.is_some() || task.trigger_on_stdout.is_some();
    ...
    if has_stdout_logic {
        ...
        // filling OS buffers; log lines at debug and forward them to stdout.
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                // Forward command stdout to the program stdout.
                println!("[run {}][{}] {}", run_id, task_name, line);
                debug!(task = %task_name, run_id, "stdout: {}", line);
            }
        });
    }
...
fn spawn_stdout_monitor(
    ...
    tokio::spawn(async move {
        ...
        debug!(task = %task_name, run_id, "stdout monitor ended");
    });
}
